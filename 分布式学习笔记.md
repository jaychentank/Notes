...
* 整体还是沿着Lab3 那个模式的骨架来，不过就是原来的kvDB是被以Shard形式切分再多个组中，Client需要取对应的组中Get, Put了。
* 需要设计如何实现数据的分片存储。
* 需要设计如何进行高效的数据迁移。
* 如何协同多个Group之间的一致性，保证数据迁移的同时不会读取到脏数据和旧数据。
...
* 根据上述Command共同处理流程，我们设计一个Loop来定时向ShardCtrlerQuery新的Config。如果Config.Num大于自身，则提交一个 NewConfigOp 给Raft, 让大家在Raft Apply之后一起安装这个NewConfig,保证一致性。
...
2. 如何监听并触发分片迁移
   ...
   if NewConfig.Shards[X] == kv.me 说明X是我新负责的，应该接受
   if OldConfig.Shards[X] == kv.me 说明X是我应该发送给别人的
   否则与我无关
   需要有发送的数据就触发数据迁移

3. 如何和Group Follower同步迁移结果
   数据迁移无非就是Leader做两件事： ”发送不再属于我的数组到它的新家，然后我自身删除这个数据，并把删除数据这个状态同步给大家“（push） & ”接受别人给我发的数据，并把它同步给大家“（pull）。 发送和接受分片数据需要我们设计一个MigrateShards RPC，而同步数据则需要我们能通过Raft进行。
   **被拉取集群的cfgNum只要比主动拉取集群的cfgNum大即可返回数据。**

4. 如何保证迁移中拒绝服务，避免脏读和旧数据
   我们设计一个
...
5. pullShard情况下，删除不属于本集群分片内容的架构设计方案：
