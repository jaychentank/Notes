...
**问题5：假设现在有S2的日志还没提交，Leader就挂掉了怎么办？**
...
**问题7：如果Leader的上层应用刚要求Leader做快照，然后发生网络分区，leader被分在了节点少的区会发生什么（也即Leader做快照需要达成共识吗）**
...
注意当Server向Raft层传输命令时会有多个channel，而Raft达成共识后通知Server层的channel只有一个applyCh
