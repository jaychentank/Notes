...
get请求可以得到最近一次完成的put请求写入的值。这种一般也被称为**强一致**（Strong Consistency）。但是，事实上，构建一个弱一致的系统也是非常有用的。**弱一致是指，不保证get请求可以得到最近一次完成的put请求写入的值**。
...
BlockStack是一种非货币使用比特币区块链。这个论文关注于如何构建一个分布式去中心化命名系统。
